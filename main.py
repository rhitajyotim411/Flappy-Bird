...
def score_display():
    sr_sf = text.render('Score: {}'.format(score), True,
                        (255, 255, 255))  # Boolean for anti-aliasing
    ...
    if game:
        ...
        sr_rt.centery = 180
        hgsr_sf = text.render('High Score: {}'.format(
            hg_sr), True, (255, 255, 255))  # Boolean for anti-aliasing
...
text = pygame.font.Font("font/04B_19.TTF", 20)
...
while True:  # game loop
    for event in pygame.event.get():  # event loop: event.get() returns the list of all events
        ...
        if event.type == FLAP:
            ...
            bird_rect = bird[bird_index].get_rect(
                center=(50, bird_rect.centery))
